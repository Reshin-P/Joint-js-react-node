...
import { dia, elementTools, shapes, util, ui,V } from "jointjs";
import $ from "jquery";
  ...
  useEffect(() => {
    ...
    if (paper) {
      ...
      paper.on("link:pointerup", (linkView) => {
        paperData.cell.push(linkView.model.attributes)
      ...
      paper.on("cell:pointerup", async function (cell) {
        ...
        let jsonObject = graph.toJSON();
        console.log("graph.tojson",jsonObject);
        let jsonString = JSON.stringify(jsonObject);
        console.log("ourData",paperData);
      ...
      stencilPaper.on("cell:pointerdown", function (cellView, e, x, y) {
        ...
        $("body").on("mouseup.fly", function (e) {
          ...
          if (
            ...
            paperData.cell.push(s.attributes)
...
const zoomIn=()=>{
  ...
  const UploadToS3 = async () => {
    const s3 = new S3({
      accessKeyId: "AKIASYHSVN43WKNXNLOJ",
      secretAccessKey: "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",
      region: "us-east-1"
    })
      ...
      <Row className="main-row ">
        ...
        <Col id="total" className="col2" sm={12} md={8} lg={8} xl={8}>
          <div className="tools">
            ...
            <Button onClick={UploadToS3} variant="outline-danger">
              OK
            </Button>
            <Button style={{marginLeft:'20px'}} onClick={zoomOut} variant="outline-warning">
